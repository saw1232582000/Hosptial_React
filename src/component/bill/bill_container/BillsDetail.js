...
import { BillItemsTable, BillsDetailInfo } from "../../bill/bill_component";
...
const BillsDetail = () => {
  ...
  const cancelBill = async () => {
    if (bill) {
      const [b, inv] = await Promise.all([
        api.put(`/api/Bill/update_bill/cancel/${id}`),
  ...
  const getDepositByPatientId = async (id) => {
    const res = await api.get(`/api/Deposit/get_deposit/active/${id}`);
    if (res.status === 200) {
  ...
  const getBill = async () => {
    const res = await api.get(`/api/Bill/get_by_id/${parseInt(id)}`);
    if (res.status === 200) {
      getDepositByPatientId(res.data.patient_id);
      setBill({ ...res.data });
  ...
  const finalize = async () => {
    const res = await api.put(`/api/Bill/update_bill/print/${parseInt(id)}`);
    if (res.status === 200) {
